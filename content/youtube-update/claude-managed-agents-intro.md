...
<strong>예시 1: Notion 내부 업무 자동화</strong>

Notion은 Managed Agents를 활용해 팀 내부 업무를 자동화했습니다.

1. Notion 보드에서 특정 카드를 "AI 처리" 상태로 이동
2. Claude 에이전트가 자동으로 태스크를 감지
3. 관련 문서를 조회하고 처리 결과를 카드에 업데이트

이 과정에서 Notion 팀이 별도로 서버나 큐(Queue, 작업 대기열) 시스템을 구축할 필요가 없었습니다. Anthropic의 인프라가 모두 처리합니다.
