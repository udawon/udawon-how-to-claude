...
Claude Code 플러그인(추가 기능을 붙이는 확장 도구)은 Claude Code에 **새로운 기능을 추가하는 확장 도구**입니다.
...
- **MCP(외부 도구 연결 기능)보다 가벼운 대안**: CLI(키보드로 명령어를 입력하는 화면) 기반이라 설정이 간단하고 속도가 빠름
...
| 3 | **Git(코드 버전 관리 도구) Worktree** | 여러 브랜치 동시 작업 | 기능 개발하면서 버그 수정 |
...
| 7 | **GitHub(코드 저장소 서비스) CLI** | PR, Issue 관리 | 터미널에서 PR 만들기 |
| 8 | **Docker(프로그램 실행 환경을 패키징하는 도구) CLI** | 컨테이너 관리 | 개발 환경 세팅 |
...
```
프롬프트(AI에게 보내는 요청 메시지): "이 프로젝트를 Taskmaster로 분석해서 작업 목록 만들어줘.
