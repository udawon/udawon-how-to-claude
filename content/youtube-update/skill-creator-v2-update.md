...
스킬 크리에이터는 **플러그인(추가 기능을 붙이는 확장 도구)**으로 설치합니다. 한 번만 설치하면 자동 업데이트됩니다.
...
| 유형 | 설명 | 예시 | 수명 |
|------|------|------|------|
| **Capability Uplift** | AI가 못하는 걸 가능하게 만드는 스킬 | PDF 생성, 특정 API(프로그램끼리 대화하는 통로) 연동 | 모델 발전 시 불필요해질 수 있음 |
...
graph TB
    ...
    style C fill:#15803d,color:#fff,stroke:#15803d,stroke-width:2px
    style E fill:#b45309,color:#fff,stroke:#b45309,stroke-width:2px
    style F fill:#1d4ed8,color:#fff,stroke:#1d4ed8,stroke-width:2px
```

<div class="example-case">
<strong>📌 실전 케이스: 스킬이 필요 없어진 경우</strong>

6개월 전 만든 "Markdown(간단한 문서 작성 형식) 표 정리" 스킬이 있다고 가정합니다.
...
| `allowed_tools` | 이 스킬이 사용할 수 있는 도구 제한 | MCP(외부 도구 연결 기능) 사용 차단 등 |
| `model` | 이 스킬 실행 시 사용할 모델 지정 | 가벼운 스킬은 Haiku로 |
| `context: fork` | 서브에이전트를 생성해서 스킬 실행 | 메인 컨텍스트 보호 |
| `hooks` | 이 스킬에만 적용되는 Hook(특정 시점에 자동 실행되는 기능) 정의 (YAML(설정 파일 작성 형식) 형식) | 스킬 실행 전/후 자동 작업 |
...
4. **결과 비교**: 토큰(AI가 처리하는 텍스트 단위) 사용량, 소요 시간, 테스트 통과율 등 비교
5. **리포트 생성**: 점수 + 개선 제안
