...
```mermaid
flowchart TD
    A["회의 발생<br/>Fireflies 녹화"] --> B["/process-meeting latest<br/>슬래시 명령어 실행"]
    B --> C["transcript-processor.md<br/>자막 로드"]
    C --> D{"액션 아이템<br/>발견?"}
    D -->|Yes| E["action-tracker<br/>스킬 호출"]
    E --> F["auto-followup<br/>스킬 호출"]
    F --> G["Supabase<br/>저장"]
    D -->|No| G
