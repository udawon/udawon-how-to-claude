...
에이전트를 정의할 때 **도구, 페르소나(AI의 성격·역할 설정), 기능**을 지정합니다. 샌드박스(격리된 실행 환경)는 필요한 패키지와 **네트워크 제어**를 포함해 구성합니다. 각 세션은 독립된 격리 환경(VM/가상 컴퓨터) 안에서 실행되며, 파일시스템(파일 저장 공간) 접근, bash(터미널 명령어 실행), 웹 검색 도구가 기본으로 갖춰집니다. 작업 결과는 이벤트 스트림(실시간 데이터 흐름)을 통해 외부 앱으로 실시간 전달됩니다.
...
<strong>메모리가 없을 때 vs 있을 때</strong>
<table>
<thead><tr><th>상황</th><th>메모리 없음</th><th>메모리 있음</th></tr></thead>
<tbody>
<tr><td>다음 주 리포트</td><td>동일한 정적 가격 데이터 나열</td><td>"클라우드 컴퓨팅 비용이 지난주 대비 15% 하락"</td></tr>
<tr><td>패턴 인식</td><td>불가</td><td>과거 데이터와 비교해 변화 감지</td></tr>
</tbody>
</table>
</div>
...
> **"당신이 완료의 기준을 정하면, Claude는 거기에 도달할 때까지 작업합니다."**
