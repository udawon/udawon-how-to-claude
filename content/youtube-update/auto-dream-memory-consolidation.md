...
## 2026년 4월 추가: Underscore Mode 논란

> 출처: [YouTube — Dream Mode e Underscore Mode: Como Funcionam no Claude Code](https://youtube.com/watch?v=QmrlNA_PHdY) (2026-04-21 공개, 포르투갈어)

Auto Dream과 함께 자주 언급되는 또 다른 내부 기능이 **Underscore Mode** (언더스코어 모드)입니다. Auto Dream이 "기억을 정리하는" 기능이라면, Underscore Mode는 **Claude Code가 코드 수정 흔적을 사람 작성처럼 보이게 하는 동작 방식**을 가리킵니다.

### 무엇을 뜻하나요?

영상 제작자가 Claude Code의 오픈소스 유출 코드(2026-03-31 소스맵 유출 건)를 분석한 결과:

- 커밋 메시지·코멘트·PR 본문에서 "AI가 만들었다"는 표시를 일부러 빼는 옵션이 존재
- 변수·함수 네이밍 패턴을 **사람의 필체**에 가깝게 조정
- 오픈소스 프로젝트에 기여할 때 사람이 수정한 것처럼 보이도록 설계

### 왜 논란인가?

이건 **투명성 관점에서 문제 여지**가 있습니다:

| 관점 | 내용 |
|------|------|
| 기업 입장 | "AI 도구라는 꼬리표 없이 자연스러운 워크플로우 통합" |
| 오픈소스 커뮤니티 입장 | 코드 리뷰어가 AI 기여인지 인간 기여인지 구분할 권리 침해 가능 |
| 법적 입장 | 일부 오픈소스 라이선스·플랫폼 약관(GitHub Copilot 조항 등)에서 AI 기여 공개 요구 가능 |

영상 제작자는 Anthropic이 n8n 같은 대형 오픈소스 프로젝트에 이미 **자율 에이전트로 기여하고 있을 가능성**을 제기합니다. 다만 이는 추측이며 공식 확인된 사실은 아닙니다.

### 비개발자에게 주는 함의

원님처럼 **비즈니스 목적으로 Claude Code를 쓰는 입장**에서는:

- **내 프로젝트가 오픈소스가 아니라면 크게 신경 쓸 일 없음** — 사적 저장소 코드는 내 책임
- **공공 기여 시에는 "AI 도움을 받았다"는 표시를 수동으로 남기는 게 안전** — 커밋 메시지 하단에 `Co-Authored-By: Claude` 넣는 관행 유지
- **오픈소스 커뮤니티 분위기 체크** — 일부 프로젝트는 AI 기여를 명시적으로 금지하거나 별도 라벨을 요구

### 정리

Underscore Mode는 공식 문서화된 기능이 아니라 **유출 코드 분석에 기반한 관찰**입니다. 확정적 결론을 내리기 전에 Anthropic의 공식 입장 발표를 기다리는 게 합리적입니다. 다만 AI 기여 투명성 원칙은 개인 선택으로 유지할 수 있습니다 — Co-Authored-By 헤더를 커밋에 남기는 것만으로도 충분합니다.
