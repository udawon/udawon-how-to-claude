...
자동 실행을 기다리지 않고 직접 꿈 정리를 시작할 수도 있습니다.

```
Claude Code에게: "dream" 또는 "auto dream" 이라고 입력
```

실행되면 `/tasks`로 진행 상태를 확인할 수 있습니다. "Memory Consolidation(메모리 통합)" 작업이 표시됩니다.

<div class="example-case">
<strong>예시: 수동 Dream 실행 후 확인</strong>

```
사용자: dream

Claude Code: (메모리 통합 시작)
  Starting memory consolidation dream.
  Let me orient first...
  Reviewing 913 sessions...
  Searching for user feedback, corrections, important decisions...

사용자: /tasks

Claude Code:
  ✅ Memory Consolidation - reviewing 913 sessions
     Phase 1: Orientation (완료)
     Phase 2: Gathering Signal (진행 중)
     Phase 3: Consolidation (대기)
```

이 과정은 세션 수에 따라 5~10분 정도 걸리며, **그동안 Claude Code를 정상적으로 사용할 수 있습니다.**
...
- **`/dream` 명령어는 아직 작동하지 않을 수 있습니다** — 공식 롤아웃 전이라 "dream"이라고 자연어로 입력하는 방식을 사용하세요
