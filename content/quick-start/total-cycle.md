...
<kbd>Shift</kbd>+<kbd>Tab</kbd>으로 Plan Mode 진입 (화면 아래 "plan mode on" 확인)
...
<kbd>Ctrl</kbd>+<kbd>G</kbd> 로 플랜을 에디터에서 직접 수정 가능
마음에 안 드는 순서나 항목이 있으면 직접 편집

<kbd>Shift</kbd>+<kbd>Tab</kbd>으로 Normal Mode 복귀 후 구현 시작.
...
<kbd>Shift</kbd>+<kbd>Tab</kbd> → Plan Mode

"카페 예약 시스템 설계해줘.
 Supabase 테이블 구조, API 라우트, 컴포넌트 구조 포함.
 @SPEC.md 있으면 참고해."

마음에 들면 <kbd>Shift</kbd>+<kbd>Tab</kbd>으로 Normal Mode 전환 후 구현.
