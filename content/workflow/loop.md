...
비유: 알람 + 심부름꾼을 동시에 쓰는 것
  ...
  매 5분마다 -> 배포 상태 확인하고 끝나면 알려줘
  매 30분마다 -> PR 리뷰 상태 체크해줘
  2시간 후에  -> 테스트 결과 정리해줘
```
...
```
"파일 수정할 때마다 lint 돌려줘"   → /hooks  (이벤트)
"1시간마다 PR 상태 확인해줘"       → /loop   (시간)
"세션 시작할 때마다 TODO 보여줘"   → /hooks  (이벤트)
"지금부터 30분마다 빌드 체크해줘"  → /loop   (시간)
"테스트 실패하면 나한테 알려줘"    → /hooks  (이벤트)
"오후 3시에 배포 생각나게 해줘"    → /loop   (시간)
```
...
```
# /loop 사용 (이번 세션만, 시간 기반)
나: /loop 5m 배포 상태 확인하고 완료되면 알려줘
Claude: 5분마다 확인할게요!

# /hooks로는 어색함 — "언제" 확인할지 이벤트가 없음
```
...
```
# /hooks 사용 (영구, 이벤트 기반)
settings.json에 SessionStart 훅 설정
→ Claude Code 켤 때마다 항상 실행

# /loop로는 불가능 — 세션 시작 후에나 등록 가능
```
