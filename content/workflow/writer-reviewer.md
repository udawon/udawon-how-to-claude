...
이중 세션:
[세션 A] "결제 로직 만들어줘"
[세션 B] "@src/payment/checkout.ts 이 코드 처음 보는 사람처럼
          냉정하게 리뷰해줘. 엣지케이스, 레이스 컨디션, 보안 집중."

-> 세션 B는 이 코드가 어떻게 만들어졌는지 모름
   훨씬 냉정하고 날카로운 리뷰가 나옴
...
```
[Writer 세션]
"로그인 기능 만들어줘. JWT 토큰 방식으로."
(구현 완료)

[Reviewer 세션 - 보안 관점]
"@src/auth/ 를 보안 전문가 관점에서 리뷰해줘.
...
claude
"@.claude/worktrees/feature-payment/src/payment/ 이 코드 리뷰해줘"

-> 구현 세션은 자기 worktree에서 계속 작업
-> 리뷰 세션은 해당 파일을 읽기만 함
-> 충돌 없음
```
...
```
# 보안 리뷰
"@[파일] 보안 취약점만 집중해서 찾아줘. OWASP Top 10 기준으로."

# 성능 리뷰
"@[파일] 성능 문제가 될 수 있는 코드를 찾아줘.
 N+1 쿼리, 불필요한 루프, 메모리 누수 위주로."

# 엣지케이스 리뷰
"@[파일] 정상 케이스 말고 실패할 수 있는 케이스를 모두 나열해줘."

# 가독성 리뷰
"@[파일] 6개월 후의 나 또는 새 팀원이 이해하기 어려울 부분을 찾아줘."

# 종합 리뷰
"@[파일] 시니어 개발자가 코드 리뷰를 한다고 생각하고,
 즉시 수정 / 개선 권장 / 괜찮음으로 분류해서 리뷰해줘."
```
