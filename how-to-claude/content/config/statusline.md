...
바로 복사해서 쓸 수 있는 실전 세팅입니다.
`/statusline` 뒤에 아래 문장을 붙여서 입력하면 자동으로 설정됩니다.
...
```
📁 how-to-claude  [Sonnet]  ▓▓▓▓░░░░░░  46%
```

- 폴더명 + 모델명 + 프로그레스 바 + %

```
/statusline 현재 폴더명을 맨 앞에, 그 다음 모델 표시명, 컨텍스트 프로그레스 바(10칸 블록)와 사용률(%)을 한 줄로 보여줘
```
...
```
📁 how-to-claude  [Sonnet]  🌿 master  $0.02  ▓▓▓▓░░░░░░  46%
```

- 폴더명 + 모델명 + git 브랜치 + 세션 비용 + 프로그레스 바

```
/statusline 현재 폴더명, 모델 표시명, git 브랜치, 세션 비용(달러), 컨텍스트 프로그레스 바(10칸)와 사용률(%)을 한 줄로 보여줘
```
...
컨텍스트 사용률에 따라 색이 바뀝니다. `/clear` 타이밍을 직관적으로 알 수 있습니다.

```
📁 how-to-claude  [Sonnet]  ▓▓▓▓░░░░░░  46%   ← 초록 (여유)
📁 how-to-claude  [Sonnet]  ▓▓▓▓▓▓▓░░░  72%   ← 노랑 (주의)
📁 how-to-claude  [Sonnet]  ▓▓▓▓▓▓▓▓▓░  91%   ← 빨강 (/clear 타이밍)
```

```
/statusline 현재 폴더명을 맨 앞에, 모델 표시명, 컨텍스트 프로그레스 바(10칸 블록)와 사용률(%)을 한 줄로 보여줘. 색상은 70% 미만이면 초록, 70~90%는 노랑, 90% 이상이면 빨강으로 표시해줘.
```
...
**사전 조건**
예시에서 쓰이는 `jq`는 JSON 데이터에서 원하는 값만 뽑아내는 도구입니다. 설치가 필요합니다:
```bash
# macOS
brew install jq
